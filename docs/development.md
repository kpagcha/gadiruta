...
3. Keep the default **Now** chip or choose a later date in the calendar popover, optionally setting a
   departure time, then select **Done**. Selecting the second place should start the search and write
   readable `from` and `to` place slugs, `date`, and optional `depart_after` to the URL. Select
   **Find transport** again to refresh that same search.
